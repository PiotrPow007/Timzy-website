...
export async function runRetention(env: TimzyEnv, admin: AdminPrincipal) {
  ...
  const expiredOrderAssets = await env.DB.prepare(`SELECT id,r2_key FROM order_assets WHERE retention_until < ? OR order_id IN
    (SELECT id FROM orders WHERE status IN ('DRAFT','AWAITING_ACCEPTANCE') AND updated_at < ?)`)
    .bind(now, draftCutoff).all<{ id: string; r2_key: string }>();
  ...
  for (const asset of expiredOrderAssets.results) await env.DOCUMENTS.delete(asset.r2_key);
  const results = await env.DB.batch([
    ...
    env.DB.prepare("DELETE FROM order_assets WHERE retention_until < ? OR order_id IN (SELECT id FROM orders WHERE status IN ('DRAFT','AWAITING_ACCEPTANCE') AND updated_at < ?)").bind(now, draftCutoff),
  ...
  const summary = { documentsDeleted: expiredDocuments.results.length, verificationDocumentsDeleted: expiredVerificationDocuments.results.length, orderAssetsDeleted: expiredOrderAssets.results.length, databaseChanges: results.reduce((total, result) => total + (result.meta.changes ?? 0), 0), executedAt: now };
