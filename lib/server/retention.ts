...
export async function runRetention(env: TimzyEnv, admin: AdminPrincipal) {
  ...
  const results = await env.DB.batch([
    ...
    env.DB.prepare(`UPDATE orders SET status='EXPIRED', client_data_encrypted=NULL, client_data_hash=NULL, selection_json='{}', updated_at=?
      WHERE status IN ('DRAFT','AWAITING_ACCEPTANCE','PENDING_PAYMENT','PAYMENT_FAILED') AND updated_at < ?`).bind(now, draftCutoff),
  ...
  const summary = { documentsDeleted: expiredDocuments.results.length, databaseChanges: results.reduce((total, result) => total + (result.meta.changes ?? 0), 0), executedAt: now };
