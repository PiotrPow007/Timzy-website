...
test("all migrations apply with valid catalogue seed and database integrity", () => {
  ...
  assert.equal(sql("SELECT group_concat(currency||':'||amount_minor,',') FROM (SELECT currency,amount_minor FROM plan_prices ORDER BY currency);"), "EUR:3900,GBP:3900,PLN:12900");
  assert.equal(sql("SELECT COUNT(*) FROM plan_prices WHERE status='DRAFT' AND stripe_price_id IS NULL;"), "3"); assert.equal(sql("SELECT COUNT(*) FROM plan_translations WHERE name='Basic';"), "3");
  assert.equal(sql("SELECT currency||':'||billing_countries_json FROM markets WHERE code='UK';"), 'GBP:["GB"]');
