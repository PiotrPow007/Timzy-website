...
function clientData(overrides: Partial<ClientLegalData> = {}): ClientLegalData {
  return {
    ...
    brandName: "Ejemplo", appName: "Ejemplo App", appContactEmail: "contacto@example.es", communicationLanguage: "es", authorityConfirmed: true, companyDataConfirmed: true, ...overrides,
  };
}
...
test("the public in-app contact email is required and validated separately", () => { assert.equal(parseClientData(clientData()).appContactEmail, "contacto@example.es"); assert.throws(() => parseClientData(clientData({ appContactEmail: "invalid" })), /incomplete/); });
