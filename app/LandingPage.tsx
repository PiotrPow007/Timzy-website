...
export function LandingPage({ copy, locale }: { copy: Copy; locale: Locale }) {
  ...
  const contractLink = { en: { href: "/contract/", label: "Configure Timzy" }, pl: { href: "/pl/umowa/", label: "Skonfiguruj Timzy" }, es: { href: "/es/contrato/", label: "Configurar Timzy" } }[locale];
  return (
    <main id="top" lang={locale === "pl" ? "pl" : locale === "es" ? "es" : "en-GB"}>
      ...
      <section className="offer"><div className="offer-copy"><p className="eyebrow">{copy.offerEyebrow}</p><h2>{copy.offerTitle}</h2><p>{copy.offerBody}</p><ul>{copy.offerPoints.map(point => <li key={point}><span>✓</span>{point}</li>)}</ul><div className="hero-actions"><a className="button button--light" href="#kontakt">{copy.offerCta}<span>→</span></a><a className="text-link" href={contractLink.href}>{contractLink.label}<span>↗</span></a></div></div><div className="offer-visual offer-visual--template"><img className="offer-template-mockup" src="/assets/templates/natural-sage-phone-transparent.png" alt="Branded app login mockup in the Natural Sage style" width="1024" height="1535" loading="lazy" /></div></section>
